...
struct Color
{
    ...
    inline Color(const Color& rhs) __attribute__((always_inline))
    {
	    r = rhs.r;
	    g = rhs.g;
	    b = rhs.b;
    ...
    inline Color& operator= (const Color& rhs) __attribute__((always_inline))
    {
	    r = rhs.r;
	    g = rhs.g;
	    b = rhs.b;
    ...
    inline Color& setRGB (uint8_t nr, uint8_t ng, uint8_t nb) __attribute__((always_inline))
    {
	    r = nr;
	    g = ng;
	    b = nb;
    ...
    // allow assignment from 32-bit (really 24-bit) 0xRRGGBB color code
    inline Color& setColorCode (uint32_t colorcode) __attribute__((always_inline))
    {
	    r = (colorcode >> 16) & 0xFF;
	    g = (colorcode >>  8) & 0xFF;
	    b = (colorcode >>  0) & 0xFF;
...
class LPD8806 {
	...
	void setPixelColor(uint16_t n, const Color& color);
	...
	uint8_t clkpinmask;	// Clock PORT bitmask
	uint8_t datapinmask;	// Data PORT bitmask
